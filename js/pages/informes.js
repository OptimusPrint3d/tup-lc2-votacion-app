...
async function realizarLlamadasAPI(datos) {
  ...
  for (var key in datos) {
    if (datos.hasOwnProperty(key)) {
      ...
      if (Array.isArray(valor) && valor.length >= 6) {
        ...
        var categoriaId = valor[4];
        var distritoId = valor[6];
        var seccionId = valor[8];
  ...
  mostrarObjetosInformes(arrayDatos,datos);
}

function mostrarObjetosInformes(arrayDatos,datos){
    var contenedorTabla = document.getElementById("tbody-informe");

    var arrayDatosValores = [];
  // Recorrer las propiedades del objeto
  for (var key in datos) {
    if (datos.hasOwnProperty(key)) {
      // Obtener el valor correspondiente a la propiedad actual
      ...
      if (Array.isArray(valor) && valor.length >= 6) {
        ...
        var tipoRecuento = valor[1];
        var tipoEleccion = valor[2];
        var tipoEleccionValor = valor[3];
        var categoriaId = valor[4];
        var categoriaIdValor = valor[5];

        var distritoId = valor[6];
        var distritoIdValor = valor[7];
        var seccionId = valor[8];
        var seccionIdValor = valor[9];
    ...
    arrayDatos.forEach(element => {
      ...
      <tr>
        ...
        <td>
          <p class="texto-elecciones-chico">
            Elecciones ${anioEleccion}  | ${tipoEleccionValor}
          </p>
          <p class="texto-path-chico">
            ${anioEleccion} > ${tipoEleccionValor} > Provisorio > ${categoriaIdValor} >
            ${distritoIdValor}
          </p>
        </td>
        <td>
      ...
      `
      //contenedorTabla += template
      contenedorTabla.innerHTML += template
    });
}